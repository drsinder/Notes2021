...
namespace Notes2021.Controllers
{
    ...
    public class DataSpyController : Controller
    {
        ...
        {
            List<TZone> s = await _context.TZones.OrderBy(p => p.OffsetHours).ThenBy(p => p.OffsetMinutes).ToListAsync();
        ...
        {
            List<UserAuxData> s = await _context.UserData.ToListAsync();    //_context.ApplicationUser.ToListAsync();
